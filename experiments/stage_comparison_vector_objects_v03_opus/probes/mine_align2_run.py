...
def run(i):
    t = time.time()
    try:
        r = subprocess.run([sys.executable, str(HERE / "mine_align2.py"), str(i)],
...
def main():
    ...
    with open(ART / "mine_align2.jsonl", "w", encoding="utf-8") as out:
        for p in sorted((ART / "mine_align2_parts").glob("*.jsonl")):
            out.write(p.read_text(encoding="utf-8"))
    n = sum(1 for _ in open(ART / "mine_align2.jsonl", encoding="utf-8"))
    (ART / "mine_align2_summary.json").write_text(json.dumps(
        {"schema_version": "mine_align2/1", "research_only": True, "groups": len(keys),
