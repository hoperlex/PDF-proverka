...
#: Нормализуется тоже: сравнение имеет смысл, только если обе стороны
#: приведены к одному виду. Если сам каталог окажется symlink'ом, ненормализо-
#: ванная константа не совпала бы ни с одним разрешённым путём, и guard
#: пропускал бы вообще всё.
...
def _publishable_path(path: Path, kind: str, lane: str) -> str:
    ...
    if expected and resolved.parent == _CANONICAL_REPORT_DIR and resolved.name == expected:
        # Строка собирается из канонической формы, а не из фактического пути:
        # после нормализации она известна точно, а `relative_to` на symlink'ах
        # могла бы отказать.
        return f"{_CANONICAL_REPORT_DIR.relative_to(ROOT)}/{expected}"
