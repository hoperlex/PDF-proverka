...
Только структура, безопасная по построению:
  ...
  * отпечаток argv — HMAC-SHA256 со случайным ключом ПРОГОНА, чтобы сличать
    процессы внутри одного прогона, не раскрывая содержимого. Обычный SHA-256
    здесь не годится: у argv низкая энтропия, и значение восстанавливается
    перебором словаря (измерено — `--pin 0427` за 10 000 попыток). Платой за
    это стала невозможность сличать отпечатки РАЗНЫХ прогонов.
