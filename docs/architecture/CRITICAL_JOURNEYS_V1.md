...
операций, все в `stage-comparison` (§4.7). Двадцать шесть ссылок с номерами
строк сверены пословно, две перенесены, одно утверждение о транспорте J-07
переписано по существу.<br>
...
| ID | Journey | P | Вход → наблюдаемый результат | Покрытие | Fixture |
| --- | --- | --- | --- | --- | --- |
| J-31 | Верхнеуровневые изменения проекта П↔РД | P1 | `project_change_summary.json` → `high_level_project_changes.json` и `text_entities.json` в каталоге пары | **Ч** | нет |

В каталог попадает один путь из трёх. Критерий §2 требует сценария, который
пользователь доводит до конца: `high-level-project-changes` SPA вызывает
(`frontend/static/js/app.js:13483`), а `graphic-comparison` и `text-entities` из
интерфейса не вызываются вовсе и остаются функциями без сценария (§7).

Покрытие J-31 — **Ч**, и это измерено: 24 теста
...
через HTTP-маршрут и ни один не подменяет транспорт модели, поэтому сквозного
детерминированного прогона у journey нет.
...
| J-31 Верхнеуровневые изменения | `codex exec` (subprocess, JSON-режим) | `services/stage_comparison/store.py:1200` → `services/llm/codex_runner.py:709`; модель — литерал `gpt-5.6-luna` в `services/stage_comparison/project_change_summary.py:21` |

Итого 7 journeys, целиком построенных вокруг модели, 1 гибрид и 2 с
LLM-подшагом. Гибрид — J-31: детерминированная маршрутизация решает, какие
группы вообще уходят в модель (`high_level_project_changes.py:418-449`),
остальные закрываются без неё (`store.py:1187`).
...
| Показатель | Значение |
| --- | --- |
| Journeys в каталоге | **31** (P0 — 21, P1 — 10) |
| Есть сквозной детерминированный тест (**Д**) | **16** |
| Покрыто частично (**Ч**) | **15** |
...
| LLM-journeys | 7 целиком + 1 гибрид (J-31) + 2 с LLM-подшагом |
| LLM-journeys, проходящих replay на записанных ответах | **0** |

**Дотягивает ли перечень до 20–30.** Да, и на каноническом срезе даже выходит за
верхнюю границу: 31 journey против диапазона 20–30. Границу трактуем как порядок
величины, а не как потолок — вычёркивать реально существующий сценарий ради
попадания в вилку значит подгонять инвентарь под условие. При необходимости
каталог сокращается до 24 без потери P0: J-19, J-21, J-22, J-24, J-28, J-29 и
J-31 — приоритета P1.
...
> Из 31 критического journey 16 имеют сквозной детерминированный тест, 15 —
...
# 31 строка каталога; шаблон строки — "| J-NN |", он не совпадает
# со ссылками вида "| J-06 Stage 01 |" в таблице §5.1
grep -cE '^\| J-[0-9]{2} \|' $F                          # 31
grep -E  '^\| J-[0-9]{2} \|' $F | grep -c '\*\*Д\*\*'    # 16
grep -E  '^\| J-[0-9]{2} \|' $F | grep -c '\*\*Ч\*\*'    # 15
grep -E  '^\| J-[0-9]{2} \|' $F | grep -c '| P0 |'       # 21
grep -E  '^\| J-[0-9]{2} \|' $F | grep -c '| P1 |'       # 10
