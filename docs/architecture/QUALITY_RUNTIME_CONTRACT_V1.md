...
профилем раннера выполнен. Шестой — исправление пятого: блокировка по ПОЛОСЕ снята обратно, оставлена
только у гейта. Причина измерена в чистой комнате: полоса не знает baseline и
падает ровно на его записях, поэтому блокировка по полосе давала бы красный CI
на каждом прогоне. Сравнение с известным долгом — работа гейта, и именно он
теперь блокирует. Пятый — регресс-гейт и полосы unit/contract/integration переведены в
БЛОКИРУЮЩИЙ режим: снят `continue-on-error` у гейта и маскировка кода 1 у
полос. Основание — исчезновение обеих причин, ради которых маскировка
стояла: группа baseline «тесты норм» закрыта починкой тестов, гейт на
чистом дереве даёт «новых 0». Полоса `network` остаётся неблокирующей с
записанным условием снятия, `chaos` — observe-only по назначению. Профиль
полос по-прежнему `--ci`: обязательность корпуса §3.3 привязана к enforce.
Четвёртый — переформулировка условия по полосе `network` после закрытия
двух узлов шлюза: полоса по-прежнему НЕ объявляется детерминированной, но
причина названа точнее — «известных узлов не осталось» не равно «не
осталось», и до блокирующего режима требуется прогон полосы под нагрузкой.
Третий — доведение поля 12 формы приёмки до CI: оба шага provisioning прокидывают
`QR_NORM_ARTIFACT_AUTH_SCHEME`, без чего небейрерная авторизация к источнику не
работала бы, а отчёт называл бы не ту причину. Переизданы пины
...
**Baseline опустошён 2026-09-03.** Сняты две последние записи — WEB
(`versions-panel`) и ENG (critic v2). Обе оказались устаревшими тестами, а не
дефектами продукта: панель версий удалена намеренно коммитом `91f48104`, а тест
critic v2 был красным с рождения — он и политика `_TAXONOMY_NO_REJECT` появились
одним коммитом `29d3a0bf`, и продукт никогда не вёл себя так, как требовала
проверка. Ни одна запись не списана: обе закрыты починкой тестов по зелёному
прямому прогону, с мутационной проверкой против тавтологии. Baseline теперь
пуст, и это самое строгое из возможных состояний: полоса и гейт краснеют на
любом падении.

**Изменение состава baseline от 2026-09-02.** Из `scripts/ci_known_failures.txt`
сняты 33 записи группы «тесты норм». Это не пересборка baseline и не подгонка
под прогон: записи сняты поимённо, по зелёному ПРЯМОМУ прогону восстановленных
тестов, и ни одна из них не зависела от корпуса норм — их причиной был отказ на
setup фикстуры. Правило §3.3 «прогон без выбранного корпуса не создаёт baseline»
не нарушено: `ci_regression_gate.py --record` не запускался, состав изменён
вручную с обоснованием в шапке файла. Оставшиеся записи принадлежат WEB и ENG.
