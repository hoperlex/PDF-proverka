...
- **Clean-room репетиция (2026-08-31):**
  ...
  бы добывать интегратору. Пять находок, из них три блокируют clean-room
  acceptance: манифесты не объявляют `pytest`; они же разрешаются в
  ...
  репозитория, и §3.2 не объявляет переменную, которая это чинит.
