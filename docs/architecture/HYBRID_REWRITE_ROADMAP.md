...
- **F10 закрыта:** `scripts/ci_regression_gate.py` получил бюджет по часам
  (`CI_GATE_WALL_BUDGET_SEC`, по умолчанию 1800 с) со своей группой процессов
  и лестницей SIGTERM → SIGKILL. Превышение — отказ с диагнозом, а не
  бесконечное ожидание. Подключение в CI и clean-room приёмка остаются за
  `W0-INT-01`.
