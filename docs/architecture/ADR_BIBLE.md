...
Три журнала различаются по назначению, владельцу и гарантиям и не подменяют друг
друга:

- **diagnostic logs** — допускают потерю и семплирование, проходят обязательную
  redaction перед записью;
- **durable audit** — append-only события безопасности и бизнес-решений с
  фиксированной схемой; потеря записи является инцидентом;
- **метрики** — только низкокардинальные labels; идентификаторы сущностей,
  пути и пользовательский ввод в labels не попадают.

Redaction является контрактом, а не соглашением: чувствительные значения,
query-параметры, presigned URL, cookies, токены, секреты и ПДн не попадают ни в
один из трёх каналов по умолчанию. Попадание регулируется явным allowlist поля,
а не отсутствием запрета. Текст исключения и traceback считаются
непроверенным пользовательским вводом и проходят ту же обработку.
...
слота на волну. Это жёсткий и непосредственно проверяемый WIP-gate. С W0
еженедельно считается доля `legacy integration hours / available human
integration hours`; пока observed baseline не появился, 20% является отчётной
целью, а не stop gate. Слот имеет пользовательский результат, владельца, срок и
парную задачу нового контура. После G1 бюджет новых legacy-capabilities равен нулю;
...
- liveness подтверждает только отзывчивость процесса и event loop: он не
  обращается к БД, storage, filesystem и внешним сервисам, отвечает минимально
  и не раскрывает диагностических данных;
- readiness проверяет зависимости, обязательные для приёма новой работы,
  использует короткие независимые timeouts и отдаёт общий отказ без деталей
  инфраструктуры;
- отказ readiness снимает узел с приёма работы, но **никогда** не является
  основанием убить процесс; право на kill/restart имеет только liveness;
- диагностический endpoint с путями, конфигурацией и версиями не совмещается с
  liveness и закрывается авторизацией.
