...
**Утверждено:** владелец программы (Древний), 2026-09-01, в задании на волну `W0-INT-01`; основание — `W0-ARC-01` выполнена, target layout нужен как frozen input интеграции<br>
...
#### Что приёмка ADR НЕ разрешает

Принятие этого ADR — одно из ТРЁХ условий перехода к шагу `contract`, а не сам
переход. Таблица выше требует ещё двух: ADR-0002 ратифицирован и D-1 закрыта.

ADR-0002 остаётся `proposed`. Значит шаг `contract` НЕ начат, конвейер стоит на
`baseline`, и никакие вытекающие из него необратимые действия — skeleton,
`pyproject.toml`, перевод `pytest.ini` под `W1-INT-00` — не разрешены. Приёмка
здесь означает ровно то, что содержание ADR принято как frozen input интеграции:
целевая раскладка, зоны владения и правила импортов.

D-1 закрывается самой приёмкой в пользу src-layout — так записано в §4.7
(«принятие ADR закрывает её первым действием»). Это следствие решения владельца,
а не отдельное решение.

Ограниченное pre-G1 владение `W0-INT-01`, описанное выше, живёт по той же
логике: оно перечисляет конкретные файлы и не открывает шаг `contract`.
